...
public class DashboardClientsCreateController {
    @FXML
    ...
    public void onAddClientButtonClicked(MouseEvent mouseEvent) {
        ...
        Response clientResponse = clientCRUDService.createClient(clientCreateDTO);// criar na base de dados

        if (clientResponse.getStatus().equals(Responses.Client.Create.CREATED_SUCCESS)) {
            Alert alert1 = new Alert(Alert.AlertType.INFORMATION, "Cliente criado com sucesso");
            alert1.showAndWait();
        } else {
            Alert alert1 = new Alert(Alert.AlertType.ERROR, "Ocorreu um erro ao criar o Cliente");
            alert1.showAndWait();
        }
    }
}
